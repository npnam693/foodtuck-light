  ...
  return (
    <header ref={headerRef} className={`flex items-center h-16 px-[10vw] mt-6 sticky top-0  
          ...
          HEADER_ACTION.map((item, index) => 
              (
                <div key={index} onClick={() => item.path && navigate(item.path)}
                  className={`text-white w-10 h-10 ml-4 flex items-center justify-center rounded-full hover:bg-[#333] ${headerSticked && currentUrl === '/' && '!text-[#333] hover:bg-[#ccc] '}`} 
                > 
                  {
                    item.title !== "Cart" ? item.icon : (
                      <Badge count={3} size="small" className={`text-white text-base  ${headerSticked && currentUrl === '/' && '!text-[#333]'}`}>
                        {item.icon}
                      </Badge>
                    )
                  }  
      ...
      <div className={`lg:hidden block ml-auto text-2xl pd-4 ${currentUrl !== '/' && 'text-white'}`} onClick={() => setOpenDrawer(true)}><AppstoreOutlined /></div>
        <Drawer  placement="right" onClose={() => setOpenDrawer(false)} open={openDrawer} title={      
          ...
          <nav className="flex flex-col">
            {
              HEADER_SITEMAP.map((item, index) => (
                <Link to={item.path} key={index} className={`text-text ml-6 relative text-xl p-2
...

            {
              HEADER_ACTION.map((item, index) => (
                <div key={index} onClick={() => {item.path && navigate(item.path);  setOpenDrawer(prv => !prv)}}
                  className={`text-text rounded-full text-xl p-2 !ml-6 w-full`} 
                > 
                  {item.icon} 
                  <span className="ml-2">
                    {item.title}
