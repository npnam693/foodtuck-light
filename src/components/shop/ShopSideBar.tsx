            ...
            <div className='flex flex-col gap-y-4'>
                {
                    listCategory.map((item, index) => (
                        <Checkbox key={index} onChange={() => {}} className='text-[#333] text-lg'>{item}</Checkbox>
                    ))
                }
            </div>
        </div>
            ...
            <div className='flex flex-wrap'>
                {
                    listProductTags.map((item, index) => (
                        <span className='mr-5 text-base text-[#4f4f4f] mb-4 border-b-2 border-[#F2F2F2]' key={index}>{item}</span>
                    ))
                }
            </div>
        </div>
